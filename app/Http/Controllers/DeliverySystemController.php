...
class DeliverySystemController extends Controller
{
    ...
    {
        $resultTmp = DB::select("select ds.id, ds.phone_number, ds.email, ds.address, ds.note, ds.`order`,ds.status, ds.created_at, ds.updated_at from delivery_system ds where ds.status =1 order by ds.`order` asc;");
